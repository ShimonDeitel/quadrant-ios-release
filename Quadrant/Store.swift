...
/// StoreKit 2 — one-time non-consumable unlock (`quadrant_pro_unlock`).
...
final class Store: ObservableObject {
    static let productID = "quadrant_pro_unlock"
    ...
    private var debugForcePro: Bool {
        #if DEBUG
        return ProcessInfo.processInfo.environment["QUADRANT_FORCE_PRO"] == "1"
        #else
        return false
        #endif
    }
    ...
    init() {
        ...
        Task {
            #if DEBUG
            // QUADRANT_NO_SK skips the product fetch so headless screenshot runs (no StoreKit config /
            // no App Store account) don't trigger the system "Sign in to Apple Account" prompt.
            if ProcessInfo.processInfo.environment["QUADRANT_NO_SK"] != "1" { await loadProduct() }
            #else
            await loadProduct()
            #endif
    ...
    #if DEBUG
    /// Test-only: force the Pro flag so unit tests can exercise Pro-gated logic without a sandbox
    /// purchase. Never compiled into Release.
